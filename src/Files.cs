...
{
	private readonly string _api = "Files";
	private readonly Func<B2Options, B2Options> _authorize;
	...
	public async Task<B2File> Upload(Stream fileDataWithSHA, string fileName, B2UploadUrl uploadUrl, string contentType, bool autoRetry, string bucketId = "", Dictionary<string, string> fileInfo = null, bool dontSHA = false,
		CancellationToken cancelToken = default)
		=> await Upload(fileDataWithSHA, new B2FileUploadContext
		{
			FileName = fileName,
			ContentType = contentType,
			BucketId = bucketId,
	...
	/// <summary>
	///     This method will assume the dontSHA property based on whether ContentSHA is supplied or not. If a ContentSHA is
	///     provided, the call will use SHA1 verification.
	/// </summary>
	...
	public async Task<B2File> Upload(byte[] fileData, B2FileUploadContext uploadContext, CancellationToken cancelToken = default)
	{
	...
	/// <param name="cancelToken"></param>
	/// <returns></returns>
	public async Task<B2FileRetentionResponse> UpdateFileRetention(string fileName, string fileId, B2FileRetentionSettings fileRetention, bool bypassGovernance = false, CancellationToken cancelToken = default)
	{
		RefreshAuthorization(_options, _authorize);
	...
	public async Task<B2DownloadAuthorization> GetDownloadAuthorization(string fileNamePrefix, int validDurationInSeconds, string bucketId = "", string b2ContentDisposition = "", CancellationToken cancelToken = default) => await GetDownloadAuthorization(fileNamePrefix, validDurationInSeconds, bucketId, b2ContentDisposition, "", "", "", "", "", cancelToken);
