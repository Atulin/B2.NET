...
public class B2Client : IB2Client
{
	...
	/// <summary>
	///     Simple method for instantiating the B2Client. Does auth for you. See
	///     https://www.backblaze.com/b2/docs/application_keys.html for details on application keys.
	...
	public async Task<B2Options> Authorize(CancellationToken cancelToken = default) => await AuthorizeAsync(_options);

	/// <summary>
	///     Only call this method if you created a B2Client with authorizeOnInitalize = false. This method of using B2.NET is
	///     considered in Beta, as it has not been extensively tested.
	/// </summary>
	/// <returns></returns>
	...
	public static async Task<B2Options> AuthorizeAsync(string keyId, string applicationkey) => await AuthorizeAsync(new B2Options { ApplicationKey = applicationkey, KeyId = keyId });

	public static B2Options Authorize(string keyId, string applicationkey) => Authorize(new B2Options { ApplicationKey = applicationkey, KeyId = keyId });

	/// <summary>
	///     Requires that KeyId and ApplicationKey on the options object be set. If you are using an application key you must
	///     specify the accountId, the keyId, and the applicationKey.
	/// </summary>
	/// <param name="options"></param>
	/// <returns></returns>
