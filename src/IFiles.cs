...
public interface IFiles
{
	...
	/// <param name="validDurationInSeconds">
	///     The number of seconds before the authorization token will expire. The minimum
	///     value is 1 second. The maximum value is 604800 which is one week in seconds.
	/// </param>
	/// <param name="bucketId"></param>
	/// <param name="b2ContentDisposition">
	///     If this is present, download requests using the returned authorization must include
	///     the same value for b2ContentDisposition
	/// </param>
	/// <param name="b2ContentLanguage">
	///     If this is present, download requests using the returned authorization must include the
	///     same value for b2ContentLanguage
	/// </param>
	/// <param name="b2Expires">
	///     If this is present, download requests using the returned authorization must include the same
	///     value for b2Expires
	/// </param>
	/// <param name="b2CacheControl">
	///     If this is present, download requests using the returned authorization must include the
	///     same value for b2CacheControl
	/// </param>
	/// <param name="b2ContentEncoding">
	///     If this is present, download requests using the returned authorization must include the
	///     same value for b2ContentEncoding
	/// </param>
	/// <param name="b2ContentType">
	///     If this is present, download requests using the returned authorization must include the
	///     same value for b2ContentType
	/// </param>
	/// <param name="cancelToken"></param>
	/// <returns></returns>
	Task<B2DownloadAuthorization> GetDownloadAuthorization(string fileNamePrefix, int validDurationInSeconds, string bucketId = "",
		string b2ContentDisposition = "", string b2ContentLanguage = "", string b2Expires = "", string b2CacheControl = "", string b2ContentEncoding = "", string b2ContentType = "", CancellationToken cancelToken = default);
	...
	/// <summary>
	///     EXPERIMENTAL: This functionality is not officially part of the Backblaze B2 API and may change or break at any
	///     time.
	///     This will return a friendly URL that can be shared to download the file. This depends on the Bucket that the file
	///     resides
	///     in to be allPublic.
	/// </summary>
	/// <param name="fileName"></param>
	...
	/// <summary>
	///     BETA: Lists the names of all non-hidden files in a bucket, starting at a given name. With an optional file prefix
	///     or delimiter.
	...
	/// <param name="cancelToken"></param>
	/// <returns></returns>
	Task<B2FileList> GetListWithPrefixOrDemiliter(string startFileName = "", string prefix = "", string delimiter = "", int? maxFileCount = null, string bucketId = "", CancellationToken cancelToken = default);
	...
	///     into a stream before uploading. If you want to avoid this and control your allocations, use the Stream compatible
	///     overload.
	/// </summary>
	/// <param name="fileData"></param>
	...
	/// <param name="cancelToken"></param>
	/// <returns></returns>
	Task<B2File> Upload(byte[] fileData, B2FileUploadContext uploadContext, CancellationToken cancelToken = default);

	/// <summary>
	///     Uploads one file to B2 using a stream, returning its unique file ID. Filename will be URL Encoded. If auto retry
	///     is set true it will retry a failed upload once after 1 second. If you don't want to use a SHA1 for the stream set
	///     dontSHA.
	/// </summary>
	...
	/// <param name="uploadUrl"></param>
	/// <param name="autoRetry"></param>
	/// <param name="uploadContext">
	///     Used for additional options when uploading to B2. File Locks, Legal Holds, Content
	///     Disposition, etc.
	/// </param>
	/// <param name="dontSHA"></param>
	/// <param name="cancelToken"></param>
	/// <returns></returns>
	Task<B2File> Upload(Stream fileDataWithSHA, B2FileUploadContext uploadContext, bool dontSHA = false, CancellationToken cancelToken = default);

	/// <summary>
	///     Copy or Replace a file stored in B2. This will copy the file on B2's servers, resulting in no download or upload
	///     charges.
	/// </summary>
	/// <param name="sourceFileId"></param>
	/// <param name="newFileName"></param>
	/// <param name="metadataDirective">
	///     COPY or REPLACE. COPY will not allow any changes to File Info or Content Type. REPLACE
	///     will.
	/// </param>
	/// <param name="contentType"></param>
	/// <param name="fileInfo"></param>
	...
	/// <param name="bypassGovernance">
	///     Must be specified and set to true if deleting an existing governance mode retention
	///     setting or shortening an existing governance mode retention period
	/// </param>
	/// <returns></returns>
	Task<B2FileRetentionResponse> UpdateFileRetention(string fileName, string fileId, B2FileRetentionSettings fileRetention, bool bypassGovernance = false, CancellationToken cancelToken = default);
}
