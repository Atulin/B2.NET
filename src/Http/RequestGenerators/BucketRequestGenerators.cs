...
public static class BucketRequestGenerators
{
	...
	///     (optional) When set, the update will only happen if the revision number stored in the B2
	///     service matches the one passed in. This can be used to avoid having simultaneous updates make conflicting changes.
	/// </param>
	/// <returns></returns>
