...
public class FileTests : BaseTest {
	private B2Bucket TestBucket = new();
	private B2Client Client;
	private static readonly string BucketPrefix = "B2NETTestingBucket";
	private static readonly string BucketName = $"{BucketPrefix}-{Path.GetRandomFileName().Replace(".", "").Substring(0, 6)}";
	private HttpClient HttpClient;

#if NETFULL
		private string FilePath => Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "../../../");
#else
	private string FilePath => Path.Combine(AppContext.BaseDirectory, "../../../");
#endif
	...
	public async Task Initialize() {
		...
		foreach (var b2Bucket in buckets) {
	...
	public async Task FileUploadTest() {
		var fileName = "B2Test.txt";
		var fileData = File.ReadAllBytes(Path.Combine(FilePath, fileName));
		var hash = Utilities.GetSHA1Hash(fileData);
		var uploadUrl = await Client.Files.GetUploadUrl(TestBucket.BucketId);
		var file = await Client.Files.Upload(fileData, new B2FileUploadContext {
			FileName = fileName,
			B2UploadUrl = uploadUrl
	...
	public async Task FileUploadStreamTest() {
		...
		var hash = Utilities.GetSHA1Hash(bytes);
	...
	public async Task FileUploadStream_WithContext_Test() {
		...
		Assert.AreEqual("1234", response.Headers.GetValues("x-bz-info-test-info").First());
	}
	...
	public static async Task Cleanup() {
		...
		foreach (var testingBucket in testingBuckets) {
			...
			foreach (var b2File in list.Files) {
				var allVersions = await client.Files.GetVersions(b2File.FileName, bucketId: testingBucket.BucketId);
